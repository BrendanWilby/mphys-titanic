...
# from ProjectScript_preprocessing import *
"------------------------------------------------------"
...
def Readangles(Number_images, name_input):
    ...
    for i in range(0, Number_images):
        hdulist = fits.open(file_names[i], ignore_missing_end=True, verbose=False)
        angles[0, i] = hdulist[0].header['PARANG']    #applies a deformation correction for Keck data

    #-0.252 degrees for before 13th of April, 2015. -0.262 for after.
    ...
    #the following creates a file containing the parameter 'ROTDEST'
    #that details the rotation correction (if any) required

    ang_correction = np.zeros((1, Number_images))
    for i in range(0, Number_images):
        hdulist = fits.open(file_names[i], ignore_missing_end=True, verbose=False)
        ang_correction[0, i] = hdulist[0].header['ROTDEST']

    np.savetxt('{name}_rotations.txt'.format(name=name_input), ang_correction)
...
def CenteringLoop(cube_orig, angs, psf):
    ...
    print(" 1. Use a 2D Gaussian Fit (have installed all useful packages)")
    print(
    ...
    print("Using DS9, open any image in your cube")
    star_xy[0] = input("Input the x coordinate of the central position of the star: ")
    star_xy[0] = int(star_xy[0])

    star_xy[1] = input("Input the y coordinate of the central position of the star: ")
...
def AnnularPCA(cube, angs, fwhm, name_input):
    ...
    ann_pca_output = vip.pca.pca_local.pca_annular(
        cube, angs, fwhm=fwhm, ncomp=15, asize=4, radius_int=mask_center,
    ...
    if questionsave == 0:
        vip.fits.write_fits('new_APCA_{name}.fits'.format(name=name_input), ann_pca_output[2], verbose=True)
        vip.fits.write_fits('new_APCA_r_{name}.fits'.format(name=name_input), ann_pca_output[1], verbose=True)
...
def StimMap(residuals_cube, name_input, origin_algo):
    ...
    if questionsave == 0:
        vip.fits.write_fits('new_STIM_{algo}_{name}.fits'.format(algo=origin_algo, name=name_input), stim_map,
                            verbose=True)
...
def ContrastCurves(cube=None, angs=None, psf=None, fwhm=None, starname=None, param=None, algo=None):
    ...
    starphot = 2.325084212649358809e+07

    # starphot_array = np.full(500, starphot)
    # np.savetxt('%s_starphot' % starname, starphot_array)

    print(" Building contrast cruves...")

    psf_norm: object = vip.metrics.fakecomp.normalize_psf(psf, fwhm, size=101, threshold=None, mask_core=None)
    ...
    if (algo == 'PCA'):
        ...
        contrast_curve = vip.metrics.contrcurve.contrast_curve(
            ...
            debug=True, save_plot='ANN_PCA', student=True, radius_int=mask_center,
            asize=fwhm/2, ncomp='auto',nproc=1
        )
        Contrastcurvedata(contrast_curve, pxscale_keck, sigma, name_input, 'ANN_PCA')
...
def Andromeda(cube, angs, psf, fwhm):
    ...
    vip.fits.write_fits('ANDR_contrastnorm_stddev_{name}.fits'.format(name=name_input), stdcontrast_norm_andr,
                        verbose=True)
...
"""
print("Project script 12\n")
name_input = input('Please enter the name of the star (example = HIP_544):')
...
"""
	------ 2 - CREATE DATA CUBE
"""
print("Thank you\nWould you like to create the data cube?")
...
if cent_loop == 0:
    cube_orig = vip.fits.open_fits('./raw_cube.fits')
    re_loop = 0
    while re_loop == 0:
...
# STIM Map
StimMap(ffpca_residuals, name_input, "PCA")
#StimMap(pca_residuals, name_input, "ANN_PCA")
...
#ContrastCurves(cube=cube, angs=angs, psf=psf, fwhm=fwhm, starname=name_input, param=pca_residuals, algo='ANN_PCA')
#llsg_rank = 6
