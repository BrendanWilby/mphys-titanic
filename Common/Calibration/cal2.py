...
for i in range(0, Number_stars):
	os.chdir('HIP{name}/{date}'.format(name=star_names[i], date=star_epochs[i]))
	
	image_filename = np.loadtxt('HIP_{name}_filenames.txt'.format(name=star_names[i]), dtype=str, max_rows=2)
