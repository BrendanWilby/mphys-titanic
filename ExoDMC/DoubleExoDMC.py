...
class exodmc(object):
	...
	def DImode(self, xlim, ylim, lxunit='au', lyunit='Mjup', verbose=True, plot=False, savefig=False, ):
		...
		for ll in range(ns):
			...
			if plot is True:
				...
				#plt.contour(sma_99542, M2_99542, map_T_99542, levels, cmap='winter', zorder=10, linewidths=1)
				
				plt.title('HIP '+self.ID[ll]+' Sensitivity Map')
				if savefig is True: plt.savefig(self.ID[ll]+'_detprob.png', dpi=300)
					
				
		return self.sma, self.M2, map.T*100
