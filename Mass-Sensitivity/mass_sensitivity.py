...
def load_contrast_data(filename):
    ...
    contrast_curve[:,0] = data[:,seperation_column] #angular distance column of contrast output
    contrast_curve[:,1] = data[:,contrast_column] #sensitivity column of contrast output
    #print(contrast_curve[0])
...
def save_data(name_data, rad_mass):
    ...
    data_file = os.path.join(final_dir, r'adi_calibrated_mass_sensitivity_data_%s.txt' %(str(int(name_data[i,0]))))
    fig_file = os.path.join(final_dir, r'adi_calibrated_mass_sensitivity_data_%s.png' %(str(int(name_data[i,0]))))
